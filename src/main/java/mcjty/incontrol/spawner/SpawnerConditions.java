...
public class SpawnerConditions {
    ...
    public static final BiFunction<TriFunction<Level, BlockPos, Player, Boolean>, TriFunction<Level, BlockPos, Player, Boolean>, TriFunction<Level, BlockPos, Player, Boolean>> AND_COMBINER = (old, extra) -> {
        if (old == null) {
            return extra;
        }
        return (level, pos, player) -> old.apply(level, pos, player) && extra.apply(level, pos, player);
    };
    public static final BiFunction<TriFunction<Level, BlockPos, Player, Boolean>, TriFunction<Level, BlockPos, Player, Boolean>, TriFunction<Level, BlockPos, Player, Boolean>> OR_COMBINER = (old, extra) -> {
        if (old == null) {
            return extra;
        }
        return (level, pos, player) -> old.apply(level, pos, player) || extra.apply(level, pos, player);
    ...
    public static class Builder {
        ...
        public Builder positiveCheck(PositionCheck positiveCheck) {
            this.positiveCheck = positiveCheck;
            return this;
        }

        public Builder negativeCheck(PositionCheck negativeCheck) {
